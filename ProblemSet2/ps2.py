...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        y = int(pos.getY())
        position = [x, y, 1]
        if position in self.room:
            i = self.room.index(position)
            self.room[i][2] = 0
            self.clean += 1
            return True
        # else:
            # raise ValueError('The tile does not exist or has been cleaned')
    ...
    def getRandomPosition(self):
        ...
        """
        random_w = random.uniform(0, (self.width - 0.01))
        random_h = random.uniform(0, (self.height - 0.01))
    ...
    def isPositionInRoom(self, pos):
        ...
        if pos.getX() < 0 or pos.getY() < 0:
            return False
        pos = [int(pos.getX()), int(pos.getY())]
        x = [i for i in range(len(self.room)) if pos[0] == self.room[i][0]]
        for i in x:
            if self.room[i][1] == pos[1]:
                return True
        return False
...
class Robot(object):
    ...
    def __init__(self, room, speed):
        ...
        self.room = room
        self.speed = speed
        self.direction = random.uniform(0.00, 359.99)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    return sum(results.values()) / float(len(results))


# Uncomment this line to see how much your simulation takes on average
print runSimulation(5, 1.0, 10, 10, 0.75, 1, StandardRobot)
# print runSimulation(1, 1.0, 5, 5, 1.0, 10, StandardRobot)
# print runSimulation(1, 1.0, 10, 10, 0.75, 10, StandardRobot)
# print runSimulation(1, 1.0, 10, 10, 0.9, 10, StandardRobot)
