...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    mean_virus = [sum(x) / float(len(x)) for x in zip(*virusPop)]
    mean_resistant_virus = [sum(x) / float(len(x)) for x in zip(*resistantVirusPop)]

    pylab.plot(mean_virus)
    pylab.plot(mean_resistant_virus)
    pylab.title('ResistantVirus Simulation')
    pylab.xlabel('Time Steps')
    pylab.ylabel('Average Virus Population')
    pylab.legend(['Mean Virus Population', 'Mean Resistant Virus Population'])
    pylab.show()
